...
class CamThread(PyQt5.QtCore.QThread):
    ...
    def __init__(self, parent):
        ...
        if self.parent.control.meas_mode == "fluorescence":
            ...
            self.image_type = ["background", "probelaser", "signal"]
    ...
    def run(self):
        while self.counter < self.counter_limit and self.parent.control.active:
            ...
            if self.parent.control.meas_mode == "fluorescence":
                type = self.image_type[self.counter%2]
            elif self.parent.control.meas_mode == "absorption":
                type = self.image_type[self.counter%3]
            ...
            if self.parent.control.active:
                ...
                if type == "background":
                ...
                elif type == "probelaser":
                    self.image_pl = image - self.image_bg
                    self.img_dict = {}
                    self.img_dict["type"] = "probelaser"
                    self.img_dict["image"] = self.image_pl
                    self.signal.emit(self.img_dict)
                    ...
                    if self.parent.control.meas_mode == "fluorescence":
                        ...
                        image = image-self.image_bg
                        image[self.image_pl==0] = 1
                        self.image_pl[self.image_pl==0] = 1
                        image_post = np.divide(image, self.image_pl)
                        image_post[image_post<=0] = 1
                        image_post[np.isnan(image_post)] = 1
...
class Control(Scrollarea):
    ...
    def img_ctrl_update(self, img_dict):
        if img_dict["type"] == "background" or img_dict["type"] == "probelaser":
            img = img_dict["image"]
